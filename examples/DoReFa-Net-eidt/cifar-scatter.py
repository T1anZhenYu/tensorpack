    ...
    def build_graph(self, image, label):
        ...
        argscope(BatchNorm, momentum=0.9, epsilon=1e-4),\
            ...
            logits = (LinearWrap(image)
                      ...
                      .apply(fg,'fg1',is_training,kernel_size=4,scatter_rate=2)#模型的核心变动，用fg代替bn和activate
                      #.BatchNorm('bn1')
                      #.apply(activate)

                      .Conv2D('conv2', 64, 3, padding='SAME')
                      .MaxPooling('pool1', 2, padding='SAME')
                      .apply(fg,'fg2',training=is_training,kernel_size=4,scatter_rate=2)#注意，这里要先maxpooling再做量化。
                      ...
                      .apply(fg,'fg3',is_training,kernel_size=4,scatter_rate=2)
                      #.BatchNorm('bn3')
                      #.apply(activate)
                      # 7

                      .Conv2D('conv4', 128, 3, padding='SAME')
                      .apply(fg,'fg4',is_training,kernel_size=4,scatter_rate=2)
                      #.BatchNorm('bn4')
                      #.apply(activate)

                      .Conv2D('conv5', 128, 3, padding='VALID')
                      .apply(fg,'fg5',is_training,kernel_size=4,scatter_rate=2)

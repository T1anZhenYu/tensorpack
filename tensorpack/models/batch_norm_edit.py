    ...
    if not do_sync_bn:
        ...
        with rename_get_variable(
            ...
            if training:
                ...
                beta_ = np.repeat(np.expand_dims(beta,axis=-1),quan_points.shape[0],axis=-1)
                gamma_ = np.repeat(np.expand_dims(gamma,axis=-1),quan_points.shape[0],axis=-1)
                moving_mean_ = np.repeat(np.expand_dims(moving_mean,axis=-1),quan_points.shape[0],axis=-1)
                moving_var_ = np.repeat(np.expand_dims(moving_var,axis=-1),quan_points.shape[0],axis=-1)
                ...
                part1 = gamma_/moving_var_*quan_points
                print('part1 is ',part1[0,0])
